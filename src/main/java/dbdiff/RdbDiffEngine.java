...
import dbdiff.pojos.dbcompare.ForeignKeyDifference;
import dbdiff.pojos.dbcompare.RdbDifference;
import dbdiff.pojos.dbcompare.RdbDifferenceType;
import dbdiff.pojos.dbcompare.RdbFoundOnSide;
...
public enum RdbDiffEngine {;
     ...
     */
    public static List<RdbDifference> compareRelationalDatabase(RelationalDatabase refDb, RelationalDatabase testDb) {
        List<RdbDifference> errors = new LinkedList<RdbDifference>();
        ...
        for (RelationalTable testT : testDb.getTables()) {
            ...
            if (refT == null) {
                RdbDifference e = new RdbDifference(RdbDifferenceType.UNEXPECTED_TABLE,
        ...
        for (RelationalTable refT : refDb.getTables()) {
            if (testDb.getTableByName(refT.getName()) == null) {
                RdbDifference e = new RdbDifference(RdbDifferenceType.MISSING_TABLE,
     ...
     */
    public static List<RdbDifference> compareRelationalTables(RelationalTable refT, RelationalTable testT) {
        List<RdbDifference> errors = new ArrayList<>();
     ...
     */
    private static List<RdbDifference> comparePrimaryKeys(RelationalTable refT, RelationalTable testT) {
        List<RdbDifference> errors = new ArrayList<>();
        if (CollectionUtils.isEmpty(refT.getPkColumns())) {
            if (CollectionUtils.isNotEmpty(testT.getPkColumns())) {
                errors.add(new RdbDifference(RdbDifferenceType.UNEXPECTED_PRIMARY_KEY,
            ...
            if (CollectionUtils.isNotEmpty(refT.getPkColumns())) {
                errors.add(new RdbDifference(RdbDifferenceType.MISSING_PRIMARY_KEY,
                        "Reference primary key " + refT.getName()
                                + refT.getPkColumns() + " is missing!",
                        RdbFoundOnSide.REF));
            }
        } else if (!refT.getPkColumns().equals(testT.getPkColumns())) {
            errors.add(new RdbDifference(RdbDifferenceType.MISCONFIGURED_PRIMARY_KEY,
     ...
     */
    private static List<RdbDifference> compareColumns(RelationalTable refT, RelationalTable testT) {
        List<RdbDifference> errors = new ArrayList<>();
        ...
        for (Column testC : testT.getColumns()) {
            ...
            if (refC == null) {
                RdbDifference e = new RdbDifference(RdbDifferenceType.UNEXPECTED_COLUMN,
                ...
                if (!Objects.equal(refC.getType(), testC.getType())) {
                    ...
                    if (StringUtils.isNotEmpty(refC.getTypeName()) && refC.getTypeName().equals(testC.getTypeName())) {
                        errorType = RdbDifferenceType.COL_TYPE_WARNING;
                    } else {
                        errorType = RdbDifferenceType.COL_TYPE_MISMATCH;
                    }
                    RdbDifference e = new RdbDifference(errorType,
                ...
                if (!Objects.equal(refC.getDefault(), testC.getDefault())) {
                    RdbDifference e = new RdbDifference(RdbDifferenceType.COL_DEFAULT_MISMATCH,
                ...
                if (!Objects.equal(refC.getIsNullable(), testC.getIsNullable())) {
                    RdbDifference e = new RdbDifference(RdbDifferenceType.COL_NULLABLE_MISMATCH,
                ...
                if (refC.getColumnSize() != null && testC.getColumnSize() != null && !refC.getColumnSize().equals(testC.getColumnSize())) {
                    RdbDifference e = new RdbDifference(RdbDifferenceType.COL_SIZE_MISMATCH,
                ...
                if (!Objects.equal(refC.getOrdinal(), refC.getOrdinal())) {
                    ...
                    RdbDifference e = new RdbDifference(RdbDifferenceType.COL_ORDINAL_MISMATCH,
        ...
        for (Column refC : refT.getColumns()) {
            if (testT.getColumnByName(refC.getName()) == null) {
                RdbDifference e = new RdbDifference(RdbDifferenceType.MISSING_COLUMN,
     ...
     * @return a {@link ForeignKeyDifference} specific to the foreign key.
     */
    private static ForeignKeyDifference getUnexpectedFkError(ForeignKey testFk, RelationalTable testT, RelationalTable refT) {
        ...
        if (!refFksByName.isEmpty()) {
            for (ForeignKey refFk : refFksByName) {
                if (refFk.equalsFrom(testFk) && refFk.equalsReference(testFk)) {
                    if (refFk.getKeySeq().equals(testFk.getKeySeq())) {
                        ...
                        return new ForeignKeyDifference(RdbDifferenceType.UNKNOWN_FK_DIFF,
                        ...
                        return new ForeignKeyDifference(RdbDifferenceType.FK_SEQUENCE_MISMATCH,
            ...
            return new ForeignKeyDifference(RdbDifferenceType.MISCONFIGURED_FK,
            ...
            if (!refFksByRefCol.isEmpty()) {
                for (ForeignKey refFk : refFksByRefCol) {
                    if (refFk.equalsFrom(testFk)) {
                        ...
                        if (refFk.getFkName().equals(testFk.getFkName())) {
                            ...
                            return new ForeignKeyDifference(RdbDifferenceType.MISNAMED_FK,
                ...
                //Unexpected FK
                return new ForeignKeyDifference(RdbDifferenceType.UNEXPECTED_FK,
     ...
     */
    private static List<RdbDifference> compareForeignKeys(RelationalTable refT, RelationalTable testT) {
        List<RdbDifference> errors = new ArrayList<>();
        Set<ForeignKey> refFks = new HashSet<>(refT.getFks());

        for (ForeignKey testFk : testT.getFks()) {
            if (!refFks.remove(testFk)) {
                ForeignKeyDifference error = getUnexpectedFkError(testFk, testT, refT);
        ...
        for (ForeignKey fk : refFks) {
            errors.add(new RdbDifference(RdbDifferenceType.MISSING_FK, "Reference foreign key \"" + fk + "\" is missing!",
                    RdbFoundOnSide.REF));
        }
        return errors;
    }
     ...
     */
    private static List<RdbDifference> compareIndices(final RelationalTable refT, final RelationalTable testT) {
        ...
        for (final Entry<List<String>, Collection<RelationalIndex>> entry : testT.getIndicesByColumns().asMap().entrySet()) {
            ...
            if (CollectionUtils.isEmpty(matchingRefIndices)) {
                for (RelationalIndex testIndex : entry.getValue()) {
                    errors.add(new RdbDifference(RdbDifferenceType.UNEXPECTED_INDEX,
                ...
                if (refIndicesWithUnknownNames == 0 && !testIndexNames.isEmpty()) {
                    for (String testIndexName : testIndexNames) {
                        errors.add(new RdbDifference(RdbDifferenceType.UNEXPECTED_INDEX, "Test index \""
                ...
                if (testIndicesWithUnknownNames == 0 && !refIndexNames.isEmpty()) {
                    for (String refIndexName : refIndexNames) {
                        errors.add(new RdbDifference(RdbDifferenceType.MISSING_INDEX, "Reference index \""
                                + getIndexDescription(refIndexName, entry.getKey(), refT) + "\" is missing!",
                                RdbFoundOnSide.REF));
                    }
                } else if (refIndexNames.size() > testIndicesWithUnknownNames) {
                    errors.add(new RdbDifference(RdbDifferenceType.MISSING_INDEX,
